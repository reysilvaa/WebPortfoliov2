	...
	async function handleSubmit(e: Event) {
		...
		try {
			const res = await fetch('/api/experiences', {
				...
				body: JSON.stringify({ role, company, startDate, endDate, description })
			});
			if (res.ok) {
				role = '';
				company = '';
		...
		<Card title="Add Experience" description="Add a new experience.">
			<form onsubmit={handleSubmit} class="space-y-6">
				<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
					<Input bind:value={role} label="Role" placeholder="e.g., Software Engineer" required />
					<Input
						bind:value={company}
						label="Company"
						...
						required
					/>
					<Input bind:value={startDate} label="Start Date" placeholder="e.g., Jan 2022" required />
					<Input bind:value={endDate} label="End Date" placeholder="e.g., Present" />
				</div>
				<Input bind:value={description} label="Description" placeholder="What did you do there?" />
		...
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
			{#each data.experiences as exp (exp.id)}
				...
				>
					...
					<Button variant="danger" size="icon" onclick={() => deleteEntry(exp.id)}>
						<svg
